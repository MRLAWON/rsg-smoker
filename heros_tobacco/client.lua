...
AddEventHandler('prop:cigar', function()
    ...
    local cigaret = CreateObject(GetHashKey('P_CIGAR01X'), x, y, z + 0.2, true, true, true)
    local righthand = GetEntityBoneIndexByName(ped, "SKEL_R_Finger12")
    ...
    if male then
        while IsEntityPlayingAnim(ped, "amb_rest@world_human_smoking@male_c@base", "base", 3)
            ...
            if IsControlJustReleased(0, Config.Prompts.DropKey) then
                ...
                SetEntityVelocity(cigar, 0.0, 0.0, -1.0)
                Wait(1500)
                ClearPedSecondaryTask(ped)
                ClearPedTasks(ped)
                Wait(10)
...
-- AddEventHandler('prop:cigar', function()
    ...
    -- local ped = PlayerPedId()
    -- local dict = 'amb_rest@world_human_smoking@male_c@stand_enter'
    -- local anim = 'enter_back_rf'
    -- local x, y, z = table.unpack(GetEntityCoords(ped, true))
